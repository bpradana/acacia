...
const TabTreeItem: React.FC<TabTreeItemProps> = ({
  ...
  const [animationDuration, setAnimationDuration] = React.useState<
    string | null
  >(null);
  const [animationDelay, setAnimationDelay] = React.useState<string | null>(
    null,
  );
  ...
  const availableLabelWidth = React.useMemo(() => {
    const maxWidth =
      sidebarWidth - paddedIndent - LABEL_RESERVED_WIDTH - LABEL_RIGHT_PADDING;
  ...
  const measureOverflow = React.useCallback(() => {
    ...
    if (overflowAmount > 4) {
      ...
      const durationSeconds = Math.min(
        Math.max(
          distance / MARQUEE_BASE_SPEED_PX_PER_SEC,
  ...
  const labelStyle = React.useMemo(
    () =>
      ({
        ...
        paddingRight: `${LABEL_RIGHT_PADDING}px`,
        "--tab-label-offset": overflowOffset,
        ...(animationDuration
          ? { "--tab-label-duration": animationDuration }
          : {}),
